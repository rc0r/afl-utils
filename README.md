...
scripts is to automate crash sample classification (see screenshot below). In version 1.01a
crash sample de-duplication using backtrace hashes generated by exploitable was introduced.
In order to use this feature invoke `afl_collect` with `-e <gdb_script>` switch for automatic
gdb+exploitable script generation and execution.  
